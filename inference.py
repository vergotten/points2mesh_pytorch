...
def init():
    global result_dir
    result_dir = os.path.join(cfg.exp_path, cfg.result_path,
...
def inference(model, model_fn, data_name, epoch):
    logger.info('>>>>>>>>>>>>>>>> Start Evaluation >>>>>>>>>>>>>>>>')

    if cfg.task == 'inference':
        from data.scannetv2_inst import Dataset
        dataset = Dataset()
        dataset.inferenceLoader()
    else:
        print("Error: no data loader - " + data_name)
        exit(0)

    dataloader = dataset.inference_data_loader
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(dataloader):
            try:
                # test batch is 1
                # N = batch['feats'].shape[0]; print(f"N: {N}")
                N = 1

                test_scene_name = dataset.inference_files[int(batch['id'][0])]
                ...
                if epoch > cfg.prepare_epochs and cfg.save_mesh:
                    ...
                    for proposal_id in range(nclusters):
                        ...
                        mesh.export(os.path.join(result_dir, "meshes",
                                                 f"{test_scene_name}_{proposal_id}_{CAD_labels[semantic_label]}_{score:.4f}.ply"))
                        if isinstance(mesh, PolyMesh):
                            tmesh = mesh.to_trimesh()
                            tmesh.export(os.path.join(result_dir, "trimeshes",
                                                      f"{test_scene_name}_{proposal_id}_{CAD_labels[semantic_label]}_{score:.4f}.ply"))
                ...
                if epoch > cfg.prepare_epochs:
                    logger.info(
                        "instance iter: {}/{} point_num: {} ncluster: {} time: total {:.2f}s inference {:.2f}s save {:.2f}s".format(
                            batch['id'][0] + 1, len(dataset.inference_files), N, nclusters, end, end1, end3))
                else:
                    logger.info(
                        "instance iter: {}/{} point_num: {} time: total {:.2f}s inference {:.2f}s save {:.2f}s".format(
                            batch['id'][0] + 1, len(dataset.inference_files), N, end, end1, end3))
...
if __name__ == '__main__':
    ...
    ##### evaluate
    inference(model, model_fn, data_name, cfg.test_epoch)