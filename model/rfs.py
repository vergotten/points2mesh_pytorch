...
# eliminate multiple bounding boxes for the same object
def non_max_suppression(ious, scores, threshold):
    """
    Eliminates multiple bounding boxes for the same object.

    Args:
        ious: Intersection over union.
        scores: Scores for each bounding box.
        threshold: Threshold for eliminating overlapping bounding boxes.

    Returns:
        A numpy array of indices for the selected bounding boxes.
    """
...
class ResidualBlock(SparseModule):
    def __init__(self, in_channels, out_channels, norm_fn, indice_key=None):
        """
        A residual block for 3D sparse convolutional neural networks.

        Args:
            in_channels: Number of input channels.
            out_channels: Number of output channels.
            norm_fn: Normalization function.
            indice_key: Key for the indices of the sparse tensor.
        """
        super().__init__()
    ...
    def forward(self, input):
        ...
        It takes an input tensor and passes it through two branches: self.i_branch and self.conv_branch.
        The output of self.conv_branch is added to the output of self.i_branch(identity) and returned.

        Args:
            input: A SparseConvTensor representing the input to the ResidualBlock.

        Returns:
            A SparseConvTensor representing the output of the ResidualBlock.
        """
...
class UBlock(nn.Module):
    def __init__(self, nPlanes, norm_fn, block_reps, block, indice_key_id=1):
        """
        Here’s what each of the parameters in the UBlock class could represent:

            nPlanes: The number of channels in the input feature map.
            norm_fn: A normalization function that is applied to the input feature map.
            block_reps: The number of times a block is repeated.
            block: A neural network block that is repeated. (Residual or VGG)
            indice_key_id: The index of the key tensor in the input SparseConvTensor.
        """
...
class RfSNet(nn.Module):
    def __init__(self, cfg):
        ...
        m = cfg.m  # 16 or 32
        classes = cfg.classes # 25
        block_reps = cfg.block_reps # 2 (number of times a block is repeated)
        block_residual = cfg.block_residual # True
        ...
        module_map = {'input_conv': self.input_conv, 'unet': self.unet, 'output_layer': self.output_layer,
                      ...
                      'z_net': self.z_net, 'z_linear': self.z_linear, 'z_in': self.z_in, 'z_out': self.z_out,
                      'z_score': self.z_score, 'z_box': self.z_box,
                      }
    ...
    def clusters_voxelization(self, clusters_idx, clusters_offset, feats, semantics, angles, coords, fullscale, scale,
                              mode):
        '''
        ...
        clusters_points_angles = torch.index_select(clusters_angles, 0,
                                                    clusters_idx[:, 0].cuda().long())  # (sumNPoint,), float
        ...
        clusters_points_coords = clusters_points_coords * torch.index_select(clusters_scale, 0,
        ...
        clusters_points_coords = torch.cat([clusters_idx[:, 0].view(-1, 1).long(), clusters_points_coords.cpu()],
                                           1)  # (sumNPoint, 1 + 3)

        out_coords, inp_map, out_map = pointgroup_ops.voxelization_idx(clusters_points_coords,
    ...
    def forward(self, data, training_mode='train'):
        ...
        if epoch > self.prepare_epochs:
            ...
            input_feats, inp_map, proposal_angle, point_center, point_scale, proposal_semantics = self.clusters_voxelization(
                proposals_idx, proposals_offset, output_feats, semantic_scores_CAD, pt_angles, coords,
            ...
            proposal_out = pointgroup_ops.roipool(proposal_out,
                                                  proposals_offset.cuda())  # (nProposal, C) proposal-wise max_pooling
            ...
            if epoch > self.prepare_epochs_2:
                ...
                ret['proposal_bbox'] = (
                proposal_zs, proposal_angle, point_center, point_scale, residual_center, residual_scale, residual_angle)
...
def model_fn_decorator(test=False):
    ...
    def test_model_fn(batch, model, epoch):
        ...
        pt_angles_residual = torch.gather(pt_angles[:, 12:] * np.pi / 12, 1, pt_angles_label.unsqueeze(-1)).squeeze(
            1)  # [N, 12] --> [N, 1] --> [N,]
        ...
        if epoch > cfg.prepare_epochs:
            ...
            if epoch > cfg.prepare_epochs_2:
                cluster_zs, cluster_angle, cluster_center, cluster_scale, cluster_residual_center, cluster_residual_scale, cluster_residual_angle = \
                ret['proposal_bbox']
                ...
                cluster_residual_center = cluster_residual_center.detach().cpu().numpy().reshape(n_clusters, 8,
                                                                                                 3)  # [nProp, 8, 3]
                cluster_residual_scale = cluster_residual_scale.detach().cpu().numpy().reshape(n_clusters, 8,
                                                                                               3)  # [nProp, 8, 3]
                cluster_residual_angle = cluster_residual_angle.detach().cpu().numpy().reshape(n_clusters,
                                                                                               8)  # [nProp, 8]
                ...
                if cfg.retrieval:
                    ...
                    for cid in range(clusters.shape[0]):
                        if cluster_mesh_ids[cid] is None:
                            ...
                            # This line of code initializes a quaternion with an axis and angle representation. The axis parameter
                            # specifies the axis of rotation and the radians parameter specifies the angle of rotation in radians.
                            # The elements attribute returns the four elements of the quaternion in the order (w, x, y, z).
                    ...
                    cluster_meshes = \
                    model.comp_net(data, phase=1, return_mesh='generate', projection_k=cfg.k_projection)[
                        'meshes']  # (nProposal_valid, )
        ...
        if epoch > cfg.prepare_epochs:
            ...
            if epoch > cfg.prepare_epochs_2:
                loss_inp['proposal_bbox'] = (pred_zs, instance_zs, instance_zs_valid, \
                                             pred_angle, pred_center, pred_scale, pred_residual_center,
                                             pred_residual_scale, pred_residual_angle, instance_bboxes,
                                             instance_bbox_size)
    ...
    def inference_model_fn(batch, model, epoch):
        ##### prepare input and forward
        # batch {'locs': locs, 'voxel_locs': voxel_locs, 'p2v_map': p2v_map, 'v2p_map': v2p_map,
        # 'locs_float': locs_float, 'feats': feats, 'labels': labels, 'instance_labels': instance_labels,
        # 'instance_info': instance_infos, 'instance_pointnum': instance_pointnum,
    ...
    def loss_fn(loss_inp, epoch):
        ...
        if epoch > cfg.prepare_epochs:
            ...
            if epoch > cfg.prepare_epochs_2:
                ...
                pred_center = pred_center + torch.gather(pred_residual_center.view(-1, 8, 3), 1,
                                                         gt_bbox_label.unsqueeze(-1).unsqueeze(-1).repeat(1, 1,
                                                                                                          3)).squeeze(
                    1)  # [nProp, 3]
                pred_scale = pred_scale + torch.gather(pred_residual_scale.view(-1, 8, 3), 1,
                                                       gt_bbox_label.unsqueeze(-1).unsqueeze(-1).repeat(1, 1,
                                                                                                        3)).squeeze(
                    1)  # [nProp, 3]
                class_balance_weights = torch.from_numpy(CAD_weights[gt_bbox_label.detach().cpu().numpy()]).float().to(
                    gt_bbox_label.device)
                ...
                z_loss = (z_loss * gt_scores * gt_zs_valid).sum() / (
                            (gt_scores * gt_zs_valid).sum() + 1e-6)  # masked loss, also * gt_scores to make it soft.
