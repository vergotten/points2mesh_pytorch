...
# Array of 20 colors for visualization instance
COLOR20 = np.array(
...
# Array of semantic names for different classes
...
# Dictionary mapping class names to their corresponding colors
CLASS_COLOR = {
    'unannotated': [0, 0, 0],
    'floor': [143, 223, 142],
    'wall': [171, 198, 230],
...
def get_coords_color(opt):
    """
    Gets the coordinates and colors of a point cloud based on the task.

    Parameters:
    opt (object): An object containing the options for the task.

    Returns:
    tuple: A tuple containing the coordinates and colors of the point cloud.
    """
    import os; cwd = os.getcwd()
    scan_data = np.load(os.path.join(cwd+'/datasets/scannet/processed_data', opt.room_name, 'data.npz'))
...
def save(opt):
    ...
    print(f"[visualize] saved {outdir}{opt.task}.ply")
    # print(f"[visualize] saved {opt.room_name}_{opt.task}.ply")


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--result_root', help='path to the predicted results', default='exp/scannetv2/rfs/rfs_pretrained_scannet/result/epoch256_nmst0.3_scoret0.05_npointt100/')
    ...
    outdir = cwd+'/visualization/'
    os.makedirs(outdir, exist_ok=True)

    if opt.room_name == 'all':
        import glob
        
        room_names = read_txt(os.path.join(f'{cwd}/datasets/splits/', 'val.txt'))
